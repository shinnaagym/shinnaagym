...
export function DashboardMonthPicker({ monthKey }: { monthKey: string }) {
  ...
  function go(year: number, month: number) {
    ...
    router.push(`/admin/dashboard?month=${key}`);
  }
    ...
    <div className="flex flex-col gap-2 mb-6">
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <button
          ...
          className="rounded-full border border-coral/40 text-coral px-3 py-1.5 text-sm font-medium hover:bg-coral/5 transition"
        >
          이번 달
        </button>
      </div>
