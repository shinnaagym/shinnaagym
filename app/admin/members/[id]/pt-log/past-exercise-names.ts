import type { AssessmentRow, PtLogCircuit, PtLogExercise, PtLogSetGroup } from "@/lib/db";

/** PT 일지(PtLogRow)·개인 운동(PersonalExerciseRow) 둘 다 이 모양을 만족해서,
    아래 함수들은 두 타입 어디에나 그대로 쓸 수 있다. */
type ExerciseLogLike = {
  log_date: string;
  exercises: PtLogExercise[];
};

/** 회원의 PT 일지·평가지에 그동안 기록된 운동 이름을 자동완성용으로 모은다. */
export function pastExerciseNames(logs: ExerciseLogLike[], assessments: AssessmentRow[]): string[] {
...
export function pastExerciseGroups(logs: ExerciseLogLike[]): Record<string, PtLogSetGroup[]> {
...
export function pastCircuitEntries(logs: ExerciseLogLike[]): Record<string, PastCircuitEntry[]> {
