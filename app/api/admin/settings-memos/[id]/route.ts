...
import { deleteSettingsMemo, updateSettingsMemo } from "@/lib/settings-memos";
...
export async function PATCH(
  ...
  const updated = await updateSettingsMemo(idNum, content);
  if (!updated) {
    return NextResponse.json({ error: "존재하지 않는 항목입니다." }, { status: 404 });
  }
  if (before) {
    await recordUndo("설정 메모 수정", [
      { op: "update", table: "settings_memos", id: idNum, data: { content: before.content } },
    ]);
  }
