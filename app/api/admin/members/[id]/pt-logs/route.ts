...
export async function POST(
  ...
  const ptLog = await createPtLog({
    memberId: idNum,
    logDate,
    painScale: parseScale(body?.painScale),
    performanceScale: parseScale(body?.performanceScale),
    exercises: parseExercises(body?.exercises),
  });

  await recordUndo(`${member.name} PT 일지 작성`, [
    { op: "delete", table: "pt_logs", id: ptLog.id },
  ]);
